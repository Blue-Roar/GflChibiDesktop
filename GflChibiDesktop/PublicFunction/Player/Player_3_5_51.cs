...
public class Player_3_5_51 : IPlayer
{
    ...
    public void Update(GameTime gameTime)
    {
        ...
        if (App.globalValues.SelectSpineVersion != "3.5.51" || App.globalValues.FileHash != skeleton.Data.Hash)
        {
            state = null;
    ...
    public void Draw()
    {
        ...
        if (state != null)
        {
            ...
            if (entry != null)
            {
                ...
                {
                    entry.trackTime = entry.trackEnd * App.globalValues.Lock;
                    entry.TimeScale = 0;
                }
                else
                {
                    App.globalValues.Lock = (entry.trackLast % entry.trackEnd) / entry.trackEnd;
                    entry.TimeScale = App.globalValues.TimeScale;
                }
                App.globalValues.LoadingProcess = $"{ Math.Round((entry.trackTime % entry.trackEnd) / entry.trackEnd * 100, 2)}%";
            }
        }
